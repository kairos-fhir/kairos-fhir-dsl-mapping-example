...
observation {
  ...
  if (context.source[laborMapping().laborFinding().laborMethod().code()] != "CIMD_KERN") {
    return
  }
  ...
  final def patIdContainer = context.source[laborMapping().relatedPatient().idContainer()]?.find {
    "PaIdTMP" == it[ID_CONTAINER_TYPE]?.getAt(CODE) //TODO switch to PaIdTMP
  }
  ...
  context.source[laborMapping().laborFinding().laborFindingLaborValues()].each { final lflv ->
    ...
    component {
      code {
        coding {
          system = "urn:centraxx"
          code = laborValue?.getAt(CODE) as String
        }
