...
specimen {
    ...
    String[] list = ["NAPKON", "NAPKON-POP"]
    boolean containsOrgUnit = list.contains(context.source[sample().organisationUnit().code()] as String) || list.contains(context.source[sample().parent().organisationUnit().code()] as String)
    ...
    // 4a Filter HEPFIX master samples (emtpy napkon id or suffix _BB)
    if ((category == SampleCategory.MASTER && !externalSampleIdContainer) || (externalSampleIdContainer && (externalSampleIdContainer?.getAt(SampleIdContainer.PSN) as String).contains("_BB"))) {
        return
    }


    // 4b Filter HEPFIX for derived and aliquotgroups
...
// TODO this is only valid for aliquots - check available temperature and handle case if no temperature exists
    ...
    //5: storage temperature attached to sample
    /*
...
*/
        ...
        parent {
            ...
            if (SampleCategory.MASTER == context.source[sample().parent().sampleCategory()] as SampleCategory && extSampleIdParent) {
                identifier {
                    ...
                    value = (extSampleIdParent[SampleIdContainer.PSN] as String).length() == 20 ? (extSampleIdParent[SampleIdContainer.PSN] as String).substring(0, 10) : (extSampleIdParent[SampleIdContainer.PSN] as String)
                }
            }
            else {
    ...
    // Eingangsdatum
    // TODO receivedtime partly available, choose that or samplingdate, but not in case of HEP
    ...
    static String toDzhkType(final Object sourceType) {
        switch (sourceType) {
            ...
            case "SAL":
                return "NUM_speichel"
            case "NASO-A":
                return "NUM_nasen-rachenabstrich"
            case "ORO-A":
                return "NUM_rachenabstrich"
            case "Heparin":
                return "NUM_pbmc_hep"
            case ["EP", "EPPL1"]:
                return "EDTA"
            case ["CP", "Citrat"]:
                return "CIT"
            case "BLDRNASTABIL": // case ["SPT", "SPT(ind)"]:
                return "NUM_pax"
            case "EDTA":
                return "EDTAWB"
...
// Mapping of the stockProcessing codes.
    static String toDzhkProcessing(final String sourceProcessing) {
        switch (sourceProcessing) {
            case "1000g10minob":
                return "NUM_BEGINN_ZENT"
            case "1000g30smb":
                return "NUM_BEGINN_ZENT"
            case "2000 g 15 min 18°C mB":
                return "NUM_RT15min2000g"
            case "2000 g 15 min 18C":
                return "NUM_RT15min2000g"
            case "400g10minrt":
                return "NUM_BEGINN_ZENT"
...
// Mappings for container codes (Probenbehälter)
    static String toDzhkContainer(final Object sourceType) {
        switch (sourceType) {
            case ["CITRATVACUTAINER", "EDTAVACU", "eSwab", "ET 1.5", "ET2ml", "ORG", "PAXgeneRNA", "SALIVETTE", "SER", "TempusBlutRNATube", "UR"]:
                return "ORG"
            case "LVL-2D-Cryo-0.5-A":
                return "NUMCryoAliquot500"
            case "LVL-2D-Cryo-2.0-A":
                return "NUMCryoAliquot2000"
...
// https://samply.github.io/bbmri-fhir-ig/Specimen-0-specimen.json.html
// https://samply.github.io/bbmri-fhir-ig/ValueSet-StorageTemperature.html

    static String toDzhkTemp(final ctx) {
        final def temp = ctx.source[sample().sampleLocation().temperature()]
