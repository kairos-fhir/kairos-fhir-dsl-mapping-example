 ...
 */
observation {
    if (!((LaborMappingType.SAMPLELABORMAPPING == context.source[laborMapping().mappingType()] as LaborMappingType) && (["DZHKFLAB"].contains(context.source[laborMapping().laborFinding().laborMethod().code()])))) {
        return
    }
    ...
    // Filter mastersamples of HEPFIX and its familly - in test was _BB
    ...
    // Filter HEPFIX mastersample childs
            ...
            identifier {
                ...
                type {
                    coding {
                        system = "urn:centraxx"
                        code = "LIMSPSN"
                        // patIdContainer[IdContainer.ID_CONTAINER_TYPE]?.getAt(IdContainerType.CODE) as String
                    }
                }
            }
        }
        ...
        component {
            ...
            if (labFinLabVal[CATALOG_ENTRY_VALUE]) {
                valueCodeableConcept {
                    labFinLabVal[LaborFindingLaborValue.CATALOG_ENTRY_VALUE].each { final def entry ->
                        coding {
