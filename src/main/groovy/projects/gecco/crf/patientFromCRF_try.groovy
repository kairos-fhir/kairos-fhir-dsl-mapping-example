...
import de.kairos.centraxx.rest.transferunit.xml.Study
...
patient {
  final def studyMember = context.source[patient().patientContainer().studyMembers()].find {
    "SARS-Cov-2 (UMG-IKCL)" == it[StudyMember.STUDY][FlexiStudy.CODE]
  }
  if (studyMember){
    return
  }
  else {
    id = "Patient/" + context.source[patient().patientContainer().studyMembers()][StudyMember.ID]
