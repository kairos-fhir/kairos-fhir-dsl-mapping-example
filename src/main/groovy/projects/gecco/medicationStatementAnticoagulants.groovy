...
medicationStatement {
  ...
  if (context.source[laborMapping().laborFinding().laborMethod().code()] != "COVID_ACE_ANTICOAGULANTS_PROFILE_CODE") {
    return // no export
  }

  id = "Anticoagulants/" + context.source[laborMapping().laborFinding().id()]
  meta {
  ...
  final def medicationCode = context.source[laborMapping().laborFinding().laborFindingLaborValues()].find {
    "COVID_ANTICOAGULANTS_CODE" == it[LaborFindingLaborValue.LABOR_VALUE]?.getAt(LaborValue.CODE)
  }
