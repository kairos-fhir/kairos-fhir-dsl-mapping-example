...
medicationStatement {
  ...
  if (context.source[laborMapping().laborFinding().laborMethod().code()] != "COVID_ACE_INHIBITORS_PROFILE_CODE") {
    return // no export
  }

  id = "ACEInhibitors/" + context.source[laborMapping().laborFinding().id()]
  meta {
  ...
  final def medicationCode = context.source[laborMapping().laborFinding().laborFindingLaborValues()].find {
    "COVID_ACE_INHIBITORS_CODE" == it[LaborFindingLaborValue.LABOR_VALUE]?.getAt(LaborValue.CODE)
  }
