...
allergyIntolerance {
  ...
  if ((context.source[laborFinding().laborMethod().code()] as String) != "Allergen") {
    return
  }

  id = "AllergyIntolerance/" + context.source[laborFinding().id()]
  ...
  final def labMap = context.source[laborFinding().laborMappings()].find {}
  if (labMap != null) {
    patient {
...
static Map<String, Object> getLflvMap(final def finding, final Map<String, String> types) {
  ...
  types.each { final String lvCode, final String lvType ->
    final List<Map<String, Object>> lflvForLv = finding[LaborFinding.LABOR_FINDING_LABOR_VALUES].findAll { final def lflv ->
