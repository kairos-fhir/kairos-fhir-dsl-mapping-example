...
observation {
  ...
  if (context.source[laborFinding().laborMethod().category()] as LaborMethodCategory != LaborMethodCategory.VITALSIGN){
    return
  }
  ...
  final def patMap = context.source[laborFinding().laborMappings()].find {}
  if (patMap != null) {
    subject {
      reference = "Patient/" + patMap[LaborMapping.RELATED_PATIENT][PatientContainer.ID]
    }
  }

  final def sampleMap = context.source[laborFinding().laborMappings()].find {final def map ->
    map[LaborMapping.MAPPING_TYPE] == LaborMappingType.SAMPLELABORMAPPING.toString()
  }

  if (sampleMap != null) {
    specimen {
      reference = "Specimen/" + sampleMap[LaborMapping.RELATED_OID]
    }
  }
